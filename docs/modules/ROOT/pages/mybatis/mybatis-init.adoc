...
<1> 使用 `XMLStatementBuilder` 解析 `xml` 中的 `SQL` 语句
    ...
    private void bindMapperForNamespace() {
        ...
        if (namespace != null) {
            ...
            if (boundType != null && !configuration.hasMapper(boundType)) {
                ...
                configuration.addMapper(boundType); <1>
            }
        }
    }
----

<1> 使用 `Configuration#addMapper` 注册 `mapper`，
...
具体的解析在 `XMLStatementBuilder#parseStatementNode` 方法中。 .org.apache.ibatis.builder.xml.XMLStatementBuilder#parseStatementNode
